...
本章議數集之建構及諸性質。數集者，蓋數之集合也。數集之有，始於自然數。自然數之有，始於人數物之需。人數物以計其數，故有自然數也。蓋自然數者，天之所賦也。人之所創者，乃記號而已也。然現代之數學需以形式論理之，故自然數之義，需以公理定之。

數學之發展，非獨賴自然數也。蓋自然數之有，猶樹之有根本也。樹之有枝葉，賴根本而生也。數學之有他數集，賴自然數而立也。故數學之始，必自自然數論起也。

= 論自然數
#definition(title: [Dedekind-Peano 結構])[
  三元組 $(N, 0, "inc")$ 備下列性質者曰 Dedekind-Peano 結構，$N$ 集合也, $0 in N$, $"inc": N -> N_* := N without {0}$ 映射也。
  + $"inc"$ 單射也
  + $M$ 者 $N$ 之子集含 $0$ 也。若 $(forall n in M) "inc"(n) in M$，則 $M = N$ 也。
]
